...
export async function requestPhoneLogin(phone) {
  ...
  dependencies.store.set(normalisePhone(phone), { code, expiresAt: Date.now() + CODE_TTL_MS, attempts: 0, userId: user.id, email: user.email });
...
export async function verifyPhoneLogin(phone, code) {
  ...
  dependencies.store.delete(normalisePhone(phone));
