...
- **The assistant panel itself:** a floating chat panel with a **mic button for voice input** — it records the user's voice and transcribes it into the composer, where they can edit it before sending. It is **never sent automatically**. If the mic is unavailable the feature shows a "not configured" notice.
