# party.fun — app overview

How the app works: architecture, features, domain rules, and the demo runbook.
For installing and running it, see [README.md](README.md).

## Contents

- [What party.fun is](#what-partyfun-is)
- [Payments & wallet (Stripe, Test mode)](#payments--wallet-stripe-test-mode)
- [Redis (caching, rate limiting, OTP storage)](#redis-caching-rate-limiting-otp-storage)
- [Email notifications (Resend)](#email-notifications-resend)
- [Password reset & phone login](#password-reset--phone-login)
- [Deadline processing (scheduler)](#deadline-processing-scheduler)
- [Current behavior](#current-behavior)
- [Event rules](#event-rules)
- [Database (Supabase)](#database-supabase)
- [Main API routes](#main-api-routes)
- [App structure (pages)](#app-structure-pages)
- [Concepts & glossary](#concepts--glossary)
- [Hype-driven pricing](#hype-driven-pricing)
- [AI event-planning agent](#ai-event-planning-agent)
- [Demo accounts](#demo-accounts)
- [Full demo runbook](#full-demo-runbook)
...
Payments are **live**: pledging creates a real off-session `PaymentIntent`, with an idempotency key and a compensating refund if the booking fails to commit ([backend/services/checkoutService.js](backend/services/checkoutService.js)).
...
A **backend scheduler** ([services/deadlineScheduler.js](backend/services/deadlineScheduler.js)) runs three jobs on each tick (service-role key; first run ~10s after boot, then on the configured interval). It's enabled automatically when `SUPABASE_SERVICE_ROLE_KEY` is set; otherwise it logs a warning and stays off.
...
An alternative to tiered pricing where a ticket's price rises with demand instead of stepping between two fixed tiers. See [HYPE_DRIVEN_PRICING.md](HYPE_DRIVEN_PRICING.md) for the product rationale.
...
After seeding, start both servers (see [README.md](README.md)) and open `http://localhost:5173`.
