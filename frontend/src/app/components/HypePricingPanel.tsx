...
export function HypePricingPanel({ event, estimatedTotal, qty }: {
  ...
  return (
    <div className="rounded-2xl border p-5" style={{ borderColor: 'rgba(255,77,46,0.25)', background: 'rgba(255,77,46,0.06)' }}>
      ...
      <div className="mb-3 flex items-stretch overflow-hidden rounded-xl" style={{ height: 36, border: '1px solid var(--border)' }}>
        <div className="flex flex-1 items-center justify-center text-xs font-semibold" style={{ background: '#29e07a', color: '#0b0b0f' }}>
          {fmt(base)} base
        </div>
        <div className="flex flex-[2] items-center justify-center text-[10px] uppercase tracking-wide" style={{ background: 'var(--surface-2)', color: 'var(--muted-foreground)' }}>
      ...
      {estimatedTotal && qty != null && qty > 0 && (
        <div className="mt-4 flex items-baseline justify-between rounded-lg px-3 py-2" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
