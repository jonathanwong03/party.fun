...
-- Paste into the Supabase SQL editor after applying 20260623_coorganisers.sql and
...
INSERT INTO demo_events(code, id, host_email, title, description, location, address, start_at, end_at, deadline_at, early_price, greenlit_price, threshold, max_capacity, status, image_url) VALUES
...
-- ── Create the events ─────────────────────────────────────────────────────────
INSERT INTO public."EVENT"(id, "hostId", title, description, location, address, "startDate", "endDate", "imageUrl", status, "createdAt", "updatedAt")
SELECT d.id, u.id, d.title, d.description, d.location, d.address, d.start_at, d.end_at, d.image_url, d.status, now(), now()
