-- ── Remove GST entirely + finalise create_pledge ──────────────────────────────
-- Ticket prices are now assumed GST-INCLUSIVE: the buyer pays the ticket total, full stop.
-- BOOKINGS."gstAmount" is dropped (nothing in the app ever read it — it was write-only),
-- which also permanently removes the "null value in column gstAmount violates not-null
-- constraint" class of failure.
--
-- This migration is SELF-CONTAINED and idempotent: it is safe to run whether or not
-- 20260715_pledge_row_lock.sql was ever applied, and it supersedes that file. It carries:
--   1. DROP of three stale create_pledge overloads left by earlier CREATE OR REPLACE
--      migrations (which never drop other signatures). They predate the row lock, GST,
--      idempotency and university gating, so any call to them silently bypassed all of it.
--      Nothing calls them: the backend always sends 6 named args (eventService.createPledge).
--   2. DROP of the gstAmount column.
--   3. The per-event row lock (concurrency): create_pledge counts active tickets, checks
--      maxCapacity, then inserts. Under READ COMMITTED two buyers of the last ticket could
--      both pass the check and oversell. A FOR UPDATE lock on the event's EVENT_SETTINGS row
--      serialises concurrent pledges for the SAME event (different events are unaffected).
--   4. Null-safe tiered pricing: an unused tier now contributes exactly 0 instead of
--      0 * NULL (= NULL in Postgres), which used to poison v_total. A genuinely NULL price on
--      a USED tier returns a clean 'invalid_pricing' error instead of a constraint violation.
...
ALTER TABLE public."BOOKINGS" DROP COLUMN IF EXISTS "gstAmount";
